...
function getVisibleNjobSections(){
  ...
  const result = { alba:false, delivery:false, free:false };
  ...
  // 아무것도 선택 안 됐거나 직장인 단독이면 전부 표시 (기존 동작 유지)
  const isEmployeeOnly = selectedJobs.includes('employee') || selectedJobs.length === 0;
  if(isEmployeeOnly){ result.alba = result.delivery = result.free = true; }
  return result;
}
...
function initNjobSection(key){
  ...
  if(albaWrap) albaWrap.style.display = vis.alba ? 'block' : 'none';
  if(deliveryWrap) deliveryWrap.style.display = vis.delivery ? 'block' : 'none';
  if(freeWrap) freeWrap.style.display = vis.free ? 'block' : 'none';
...
function clearNjobInput(type){
  const ids = {
    alba: ['njob-alba-name', 'njob-alba-hours'],
...
function saveNjobAll(){
  ...
  if(albaName && albaWage > 0 && albaHours > 0){
    data.alba.push({id:now + 1, name:albaName, wage:albaWage, hours:albaHours});
    clearNjobInput('alba');
    saved = true;
    // 시급 자동저장
