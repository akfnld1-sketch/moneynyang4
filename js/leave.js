...
function lsSave(){
  try{
    ...
    try{ localStorage.setItem('atm2_flData', JSON.stringify(flData)); }catch(e2){}
    ...
    // 수동급여 저장
    if(activeWpId && activeEmpId) localStorage.setItem(PAY_KEY(activeWpId,activeEmpId)+'_manual', JSON.stringify(manualPay));
    // 이번달 예상 실수령액 (다음달 비교용)
    try{
      const pd = getPayData();
      if(pd && pd.finalPay > 0){
